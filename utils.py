...
def upsample_bilinear(image, ratio=4):
    h, w, c = image.shape
...
def upsample_bicubic(image, ratio=4):
    h, w, c = image.shape
...
def downsample_bilinear(image, ratio=0.25):
    h, w, c = image.shape
    re_image = cv2.resize(image, (w * ratio, h * ratio), cv2.INTER_LINEAR)
...
def downsample_bicubic(image, ratio=0.25):
    h, w, c = image.shape
    re_image = cv2.resize(image, (w * ratio, h * ratio), cv2.INTER_CUBIC)
...
# def gaussian_DOWN4(original_msi):  # 下采样4倍
